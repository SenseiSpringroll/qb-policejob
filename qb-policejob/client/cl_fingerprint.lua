...
Event = "fingerprint:client:psmdt" -- The event to trigger for your MDT. (I MADE A PS-MDT EVENT TO TRIGGER BY DEFAULT SO IF YOU USE PS-MDT, LEAVE IT AS IT IS.)
...
RegisterNetEvent('fingerprint:client:policetablet') 
AddEventHandler('fingerprint:client:policetablet', function()
    if PlayerJob.type == "leo" then
        ...
        QBCore.Functions.Progressbar("scan_finger", "Attempting to scan finger..", 5000, false, true, {
            ...
            if player ~= -1 and distance < 2.5 then
                ...
                TriggerServerEvent("fingerprint:server:fingerprintmenu", playerId)
            else
...
RegisterNetEvent('fingerprint:client:psmdt', function()
...
RegisterNetEvent('fingerprint:client:fingerprintmenu') 
AddEventHandler('fingerprint:client:fingerprintmenu', function(pdata)
name = pdata.charinfo.firstname..' '..pdata.charinfo.lastname
    if UseMDT then
        ...
        Wait(1000)
        TriggerEvent("fingerprint:copyToClipBoard", name)
    else
        exports['qb-menu']:openMenu({
            {
                header = "Fingerprint Scanner",
...
RegisterNetEvent("fingerprint:copyToClipBoard")
AddEventHandler("fingerprint:copyToClipBoard", function(data)
